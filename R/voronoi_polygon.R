...
voronoi_polygon <- function(data, x = "x", y = "y", outline = NULL, data.frame = FALSE) {
  ...
  if (!is.null(outline)) {
    sf_x <- sf::st_buffer(sf::st_as_sf(vor_spdf), dist = 0)
    sf_y <- sf::st_buffer(sf::st_as_sf(outline_spdf), dist = 0)
